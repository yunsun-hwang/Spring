...
<body>
          ...
          <c:forEach var = "a" items = "${attractions}">
	          <div class="col-xl-3 col-lg-4 col-md-6 mb-4">
	            <div class="bg-white rounded shadow-sm">
	              <img
	                src="${a.img}"
	                onerror="this.src='${root}/resources/assets/NoImage.png'"
	                alt=""
	                class="img-fluid card-img-top"
	              />	            
	              <div class="p-4">
	                <h5><a href="${root}/attr?action=detail&contentId=${a.contentId}" class="text-dark">${a.title}</a></h5>
	                <p class="small text-muted mb-0">${a.addr}</p>
	              </div>
	            </div>
	          </div>				          
          </c:forEach>
	...
	<script>
        document.querySelector("#search-area").addEventListener("change", function () {
        	requestGugun(this.value);
        });
		
        function requestGugun(id) {
            ...
            fetch("${root}/attr?action=gugun&id=" + id)
               ...
               .then(data => {
               	    ...
               	    if ('${gugun}') {
               	    	document.querySelector("#search-area2").value = '${gugun}';
               	    }
           		});	
        }
        requestGugun(document.querySelector("#search-area").value);
        
   	    if ('${type}') {
   	    	document.querySelector("#search-content-id").value = '${type}';
   	    }
    </script>
</body>
</html>